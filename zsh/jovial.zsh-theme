...
typeset -gA JOVIAL_PALETTE=(
    # hostname
    host '%F{240}'            # gris oscuro

    # common user name
    user '%F{250}'            # gris claro

    # only root user
    root '%B%F{203}'          # rojo pálido (error)
    ...
    # elapsed time of last command executed
    elapsed '%F{222}'         # amarillo tenue (warning soft)

    # exit code of last command
    exit.mark '%F{246}'       # gris claro
    ...
    # for other common case text color
    normal '%F{252}'          # blanco sucio

    success '%F{108}'         # verde suave
    error '%F{203}'           # rojo pálido
)
